...
typedef int64_t HSteamPipe;
// handle to single instance of a steam user
typedef int64_t HSteamUser;
// function prototype
#if defined(POSIX)
#define __cdecl
#endif
extern "C" typedef void(__cdecl* SteamAPIWarningMessageHook_t)(int, const char*);
extern "C" typedef void (*SteamAPI_PostAPIResultInProcess_t)(SteamAPICall_t callHandle, void*,
                                                             uint32         unCallbackSize,
                                                             int            iCallbackNum);
extern "C" typedef int64_t (*SteamAPI_CheckCallbackRegistered_t)(int iCallbackNum);
#if defined(__SNC__)
#pragma diag_suppress =                                                                            \
    1700 // warning 1700: class "%s" has virtual functions but non-virtual destructor
#endif
...
//			where you want to get different versions of the same interface in different dll's in
//your project.
//-----------------------------------------------------------------------------
class ISteamClient
{
  public:
    ...
    virtual void SetLocalIPBinding(uint32 unIP, uint16_t usPort) = 0;
    ...
    virtual ISteamUnifiedMessages* GetISteamUnifiedMessages(HSteamUser  hSteamuser,
                                                            HSteamPipe  hSteamPipe,
                                                            const char* pchVersion) = 0;
...
#define CALLBACK_MEMBER(varidx, vartype, varname)                                                  \
  public:                                                                                          \
    vartype     varname;                                                                           \
    static void GetMemberVar_##varidx(unsigned int& varOffset, unsigned int& varSize,              \
                                      uint32& varCount, const char** pszName,                      \
                                      const char** pszType)                                        \
    {                                                                                              \
...
#define END_CALLBACK_INTERNAL_BEGIN(numvars)                                                       \
    ...
    static bool GetMemberVariable(uint32 index, uint32& varOffset, uint32& varSize,                \
                                  uint32& varCount, const char** pszName, const char** pszType)    \
    {                                                                                              \
        switch (index)                                                                             \
        {                                                                                          \
        default:                                                                                   \
