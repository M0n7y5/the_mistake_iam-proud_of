...
extern "C" int __cdecl __stdio_common_vfprintf(unsigned __int64 const options, FILE* stream, char const* const format,
                                               _locale_t locale, va_list const arglist)
{
...
extern "C" unsigned long strtoul(char const* _String, char** _EndPtr, int _Radix)
{
    static auto fn = LI_FN(strtoul).in_safe(crtbaseModule.get());
    return fn(_String, _EndPtr, _Radix);
}
...
extern "C" size_t wcslen(wchar_t const* _String1)

{
    static auto fn = LI_FN(wcslen).in_safe(crtbaseModule.get());
    return fn(_String1);
}
