...
#include <intrin.h>
    ...
    static bool getX86CpuIDAndInfo(unsigned value, unsigned* rEAX, unsigned* rEBX, unsigned* rECX,
                                   unsigned* rEDX)
    {
#if defined(__GNUC__) || defined(__clang__)
    ...
    /// return true.
    static bool getX86CpuIDAndInfoEx(unsigned value, unsigned subleaf, unsigned* rEAX,
    ...
    static const char* getIntelProcessorTypeAndSubtype(unsigned Family, unsigned Model,
                                                       const unsigned* Features, unsigned* Type,
                                                       unsigned* Subtype)
    {
#define testFeature(F) (Features[F / 32] & (1 << (F % 32))) != 0
    ...
    static const char* getAMDProcessorTypeAndSubtype(unsigned Family, unsigned Model,
                                                     const unsigned* Features, unsigned* Type,
                                                     unsigned* Subtype)
    {
        // We select CPU strings to match the code in Host.cpp, but we don't use them
...
#define setFeature(F) Features[F / 32] |= 1U << (F % 32)
        ...
        bool           HasAVX =
            ((ECX & AVXBits) == AVXBits) && !getX86XCR0(&EAX, &EDX) && ((EAX & 0x6) == 0x6);
...
#undef setFeature
    ...
    int CONSTRUCTOR_ATTRIBUTE __cpu_indicator_init(void)
    {
        ...
        if (!isCpuIdSupported() || getX86CpuIDAndInfo(0, &MaxLeaf, &Vendor, &ECX, &EDX) ||
            MaxLeaf < 1)
        {
