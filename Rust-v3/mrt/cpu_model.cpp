...
#include <intrin.h>
    ...
    static bool getX86CpuIDAndInfo(unsigned value, unsigned* rEAX, unsigned* rEBX, unsigned* rECX, unsigned* rEDX)
    {
#if defined(__GNUC__) || defined(__clang__)
    ...
    /// return true.
    static bool getX86CpuIDAndInfoEx(unsigned value, unsigned subleaf, unsigned* rEAX, unsigned* rEBX, unsigned* rECX,
                                     unsigned* rEDX)
    {
#if defined(__GNUC__) || defined(__clang__)
#if defined(__x86_64__)
    ...
    static const char* getIntelProcessorTypeAndSubtype(unsigned Family, unsigned Model, const unsigned* Features,
...
#define testFeature(F) (Features[F / 32] & (1 << (F % 32))) != 0
    ...
    static const char* getAMDProcessorTypeAndSubtype(unsigned Family, unsigned Model, const unsigned* Features,
                                                     unsigned* Type, unsigned* Subtype)
    {
        // We select CPU strings to match the code in Host.cpp, but we don't use them
        // in compiler-rt.
    ...
    static void getAvailableFeatures(unsigned ECX, unsigned EDX, unsigned MaxLeaf, unsigned* Features)
    {
...
#define setFeature(F) Features[F / 32] |= 1U << (F % 32)
        ...
        bool           HasAVX  = ((ECX & AVXBits) == AVXBits) && !getX86XCR0(&EAX, &EDX) && ((EAX & 0x6) == 0x6);
        ...
        bool HasLeaf7Subleaf1 = HasLeaf7 && EAX >= 1 && !getX86CpuIDAndInfoEx(0x7, 0x1, &EAX, &EBX, &ECX, &EDX);
        ...
        bool HasExtLeaf1 = MaxExtLevel >= 0x80000001 && !getX86CpuIDAndInfo(0x80000001, &EAX, &EBX, &ECX, &EDX);
...
#undef setFeature
...
#endif
    int
    __cpu_indicator_init(void) CONSTRUCTOR_ATTRIBUTE;
    ...
    int CONSTRUCTOR_ATTRIBUTE __cpu_indicator_init(void)
    {
        ...
        if (!isCpuIdSupported() || getX86CpuIDAndInfo(0, &MaxLeaf, &Vendor, &ECX, &EDX) || MaxLeaf < 1)
        {
        ...
        {
            // Get CPU type.
            getIntelProcessorTypeAndSubtype(Family, Model, &Features[0], &(__cpu_model.__cpu_type),
        ...
        {
            // Get CPU type.
            getAMDProcessorTypeAndSubtype(Family, Model, &Features[0], &(__cpu_model.__cpu_type),
                                          &(__cpu_model.__cpu_subtype));
    ...
    _Bool __aarch64_have_lse_atomics __attribute__((visibility("hidden"), nocommon));
...
#define IF_EXYNOS9810                                                                                                  \
    ...
    if (__system_property_get("ro.arch", arch) > 0 && strncmp(arch, "exynos9810", sizeof("exynos9810") - 1) == 0)
...
#pragma comment(lib, "zircon")
        ...
        __aarch64_have_lse_atomics = status == ZX_OK && (features & ZX_ARM64_FEATURE_ISA_ATOMICS) != 0;
#else
...
#define extractBits(val, start, number) (val & ((1ULL << number) - 1ULL) << start) >> start
