...
enum EResult
{
    ...
    k_EResultDuplicateRequest = 29, // The request is a duplicate and the action has already
    ...
    k_EResultExternalAccountAlreadyLinked =
        59, // External account (PSN, Facebook...) is already linked to some other account, must
    ...
    k_EResultRateLimitExceeded = 84, // Temporary rate limit exceeded, try again later, different
...
typedef enum
{
    k_EAuthSessionResponseOK = 0, // Steam has verified the user is online, the ticket is valid and
    ...
    k_EAuthSessionResponseLoggedInElseWhere =
        4, // The user account has logged in elsewhere and the session containing the game instance
...
enum EAppReleaseState
{
    ...
    k_EAppReleaseState_Prerelease  = 2, // can be purchased and is visible in games list, nothing
                                        // else. Common appInfo section released
    k_EAppReleaseState_PreloadOnly =
        3,                           // owners can preload app, not play it. AppInfo fully released.
...
enum EAppOwnershipFlags
{
    ...
    k_EAppOwnershipFlags_RegionRestricted =
        0x0004, // owns app, but not allowed to play in current region
    ...
    k_EAppOwnershipFlags_LicensePending =
        0x0200, // owns app, but transaction is still pending. Can't install or play
    ...
    k_EAppOwnershipFlags_LicensePermanent =
        0x0800, // permanent license, not borrowed, or guest or freeweekend etc
    k_EAppOwnershipFlags_LicenseRecurring =
        0x1000, // Recurring license, user is charged periodically
    k_EAppOwnershipFlags_LicenseCanceled =
        0x2000, // Mark as canceled, but might be still active if recurring
};
...
enum EAppType
{
    ...
    k_EAppType_Media_DEPRECATED =
        0x010, // legacy - was used for game trailers, which are now just videos on the web
    ...
    k_EAppType_TVSeries =
        0x400, // A TV or other video series which will have episodes and perhaps seasons
    k_EAppType_Video = 0x800, // A video component of either a Film or TVSeries (may be the feature,
                              // an episode, preview, making-of, etc)
...
enum EChatEntryType
{
    ...
    // k_EChatEntryTypeLobbyGameStart = 5,	// lobby game is starting (dead - listen for
    // LobbyGameCreated_t callback instead)
...
// we allow 3 simultaneous user account instances right now, 1= desktop, 2 = console, 4 = web, 0 =
// all
...
class CSteamID
{
  public:
             ...
             EAccountType eAccountType)
    {
        ...
        Assert(!((k_EAccountTypeIndividual == eAccountType) &&
                 (unAccountInstance > k_unSteamUserWebInstance))); // enforce that for individual
    ...
    void FullSet(uint64 ulIdentifier, EUniverse eUniverse, EAccountType eAccountType)
    {
        m_steamid.m_comp.m_unAccountID =
            (ulIdentifier & k_unSteamAccountIDMask); // account ID is low 32 bits
    ...
    void SetFromSteam2(TSteamGlobalUserID* pTSteamGlobalUserID, EUniverse eUniverse)
    {
        ...
        m_steamid.m_comp.m_unAccountInstance =
            k_unSteamUserDesktopInstance; // Steam2 only knew desktop instances
    }
    ...
    {
        // note we do NOT include the account instance (which is a dynamic property) in the static
        // account key
    ...
    static const char* Render(uint64 ulSteamID); // static method to render a uint64 representation
                                                 // of a steam ID to a string

#ifdef CSTEAMID_RENDERLINK_SUPPORT
    const char*        RenderLink() const; // renders this steam ID to an admin console link string
    static const char* RenderLink(uint64 ulSteamID); // static method to render a uint64
  ...
  private:
    // These are defined here to prevent accidental implicit conversion of a u32AccountID to a
    // CSteamID. If you get a compiler error about an ambiguous constructor/function then it may be
    // because you're passing a 32-bit int to a function that takes a CSteamID. You should
    // explicitly create the SteamID using the correct Universe and account Type/Instance values.
    CSteamID(uint32);
    CSteamID(int32);
    ...
    union SteamID_t {
        struct SteamIDComponent_t
        {
            ...
            unsigned int m_EAccountType : 4; // type of account - can't show as EAccountType, due to
...
inline bool CSteamID::IsValid() const
{
    ...
    if (m_steamid.m_comp.m_EAccountType == k_EAccountTypeIndividual)
    {
        if (m_steamid.m_comp.m_unAccountID == 0 ||
            m_steamid.m_comp.m_unAccountInstance > k_unSteamUserWebInstance)
            return false;
    }
...
// This steamID comes from a user game connection to an out of date GS that hasnt implemented the
// protocol to provide its steamID
...
// This steamID can come from a user game connection to a GS that has just booted but hasnt yet even
// initialized its steam3 component and started logging on.
#define k_steamIDNotInitYetGS CSteamID(1, 0, k_EUniverseInvalid, k_EAccountTypeInvalid)
// This steamID can come from a user game connection to a GS that isn't using the steam
...
// If the steamID passed in is already of type k_EAccountTypeChat it will be returned with the same
// instance
CSteamID ChatIDFromSteamID(const CSteamID& steamID);
// Returns the matching clan steamID, with the default instance of 0
// If the steamID passed in is already of type k_EAccountTypeClan it will be returned with the same
// instance
...
class CGameID
{
  public:
    ...
    static const char* Render(uint64 ulGameID); // static method to render a uint64 representation
                                                // of a Game ID to a string
...
//  just before minidump file is captured after a crash has occurred.  (Allows app to append
//  additional comment data to the dump, etc.)
//-----------------------------------------------------------------------------
