...
inline void Init()
{
    ...
    // builder.AddText((char*)u8"ěščřžýáíéůúó");                        // Add a
    // string (here "Hello world" contains 7 unique characters)
    ...
    fonts::defaultFont = ImGui::GetIO().Fonts->AddFontFromFileTTF("G:\\fonts\\NotoSans-Regular.ttf",
                                                                  19.f, &cfg, ranges.Data);
    fonts::boldFont = ImGui::GetIO().Fonts->AddFontFromFileTTF("G:\\fonts\\NotoSans-Bold.ttf", 20.f,
                                                               &cfg, ranges.Data);
    fonts::logo =
        ImGui::GetIO().Fonts->AddFontFromMemoryCompressedBase85TTF(GrLogoData_base85, 50.f, &cfg);
    fonts::categoryFont =
...
// helper method for keybinds
bool GetKeyDownHelper(KeyCode code);

static inline ImVector<ImRect> s_GroupPanelLabelStack;

static inline ImFont *spacerfont;

static inline bool KeyBindActivePopup = false;

static inline KeyButton *currentKeyBind = nullptr;
...
inline void BeginGroupPanel(const char *name, const ImVec2 &size = ImVec2(0.0f, 0.0f))
{
    ...
    // ImGui::GetWindowDrawList()->AddRect(labelMin, labelMax, IM_COL32(255, 0,
    // 255, 255));
...
inline void ToggleButton(const char *str_id, bool *v)
{
    ...
    //     col_bg = ImGui::GetColorU32(ImLerp(colors[ImGuiCol_FrameBg], colors[ImGuiCol_FrameBg],
    //     t));
    // else
    ...
    col_bg =
        ImGui::GetColorU32(ImLerp((ImVec4)ImColor(0, 115, 255, 0), ImColor(0, 115, 255, 120), t));
...
inline void EndFeatureSet()
{
    ...
    ImGui::GetWindowDrawList()->AddShadowRect(min, max, IM_COL32(0, 0, 0, 255), 35, {0, 0},
                                              ImDrawFlags_ShadowCutOutShapeBackground, 7);
    ImGui::GetWindowDrawList()->AddShadowRect(min, max, IM_COL32(0, 0, 0, 50), 35, {0, 0},
                                              ImDrawFlags_ShadowCutOutShapeBackground, 7);
    // ImGui::GetWindowDrawList()->AddShadowRect(min, max, IM_COL32(0, 0, 0,
    // 255), 30, {0, 0}, ImDrawFlags_ShadowCutOutShapeBackground, 10);
                          ...
                          ImGuiSliderFlags flags = 0)
{
    ...
    if (hovered)
    {
        t_anim = ImSaturate(g.HoveredIdTimer / 0.2f);
    ...
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(
            ImGui::GetStyleColorVec4(g.ActiveId == id ? ImGuiCol_FrameBgActive : ImGuiCol_FrameBg),
    ...
    // GetColorU32(ImGuiCol_Border), 3.f);
    // window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.GetCenter().y),
    ...
    // Slider behavior
    ImRect     grab_bb;
    const bool value_changed = ImGui::SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max,
    ...
    {
        // window->DrawList->AddLine(ImVec2(frame_bb.Min.x,
        // grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10,
        // grab_bb.GetCenter().y), GetColorU32(g.ActiveId == id ?
        // ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 2.f);

        /*window->DrawList->AddRectFilledMultiColor(ImVec2(frame_bb.Min.x,
           frame_bb.Min.y + 6.f), ImVec2(grab_bb.Max.x, grab_bb.Max.y-6.f ),
            GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive :
           ImGuiCol_SliderGrab), GetColorU32(g.ActiveId == id ?
           ImGuiCol_SliderGrabActive2 : ImGuiCol_SliderGrab2),
            GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive2 :
           ImGuiCol_SliderGrab2), GetColorU32(g.ActiveId == id ?
        ...
        // auto col_2 = GetColorU32(g.ActiveId == id ?
        // ImGuiCol_SliderGrabActive2 : ImGuiCol_SliderGrab2);
        ...
        ImGui::ColorConvertRGBtoHSV(col_2_vec.x, col_2_vec.y, col_2_vec.z, col_2_vec.x, col_2_vec.y,
                                    col_2_vec.z);
        col_2_vec.z = ImMin(col_2_vec.z + h_increase, 1.0f);
        col_2_vec.z = ImMin(col_2_vec.z + v_increase, 1.0f);
        ImGui::ColorConvertHSVtoRGB(col_2_vec.x, col_2_vec.y, col_2_vec.z, col_2_vec.x, col_2_vec.y,
                                    col_2_vec.z);
...
class BoolFeature : public Feature<bool>
{
  ...
  public:
    ...
    void Draw(ImVec2 size) override
    {
        ...
        {
            ImGui::SameLine();
            auto txt     = GetKeyString(*this->key);
            auto txtSize = ImGui::CalcTextSize(txt.c_str(), 0, false);
            auto curY    = ImGui::GetCursorPosY();
            ImGui::SetCursorPosX(curX + size.x - txtSize.x - 5);
            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(255, 255, 255, 120));
            ImGui::TextUnformatted(txt.c_str(), ImGui::FindRenderedTextEnd(txt.c_str()));
            ImGui::PopStyleColor();
                                                                                ...
                                                                                // IsItemHovered()
            {
                ...
                {
                    if (hold)
                        (*key).buttonData.type = TriggerType::HOLD;
                }
                if (ImGui::Checkbox(_("Toggle"), &toggle))
                {
                    if (toggle)
...
class ColorFeature : public Feature<ImColor>
{
  ...
  public:
    ...
    void Draw(ImVec2 size) override
    {
        ...
        const auto curX = ImGui::GetCursorPosX();
        const auto curY = ImGui::GetCursorPosY();
        ...
        //         {min.x + (size.x / 2), min.y + (size.y / 2)}, size.x / 2.5,
        //         ImGui::GetColorU32(glow), 25, {0, 0});
        // }
...
template <typename T> class ComboFeature : public Feature<T>
{
...
{
  public:
    explicit ConfigFeature(const std::string &name)
    {
    }
...
class FeatureSet
{
  ...
  public:
    ...
    FeatureSet &AddFeature(std::string name, ImColor *variable, bool *togg = nullptr)
    {
        const auto f = new ColorFeature(std::move(name), variable, togg);
    ...
    template <typename T>
    FeatureSet &AddFeature(std::string name, T *_target, std::vector<T> possibleVals,
                           std::vector<std::string> stringVals)
    {
        const auto f = new ComboFeature(name, _target, possibleVals, stringVals);
        _features.emplace_back(f);
        return *this;
    }
    ...
    void Draw(ImVec2 s)
    {
        ...
        ImGui::GetWindowDrawList()->AddShadowRect(min, max, IM_COL32(0, 0, 0, 255), 45, {0, 0}, 0,
                                                  7);
...
class Category
{
    ...
    SimpleAnimation<float> hoverAnim{0, 1.f, 1.0f, STween::QuintOut};
  ...
  public:
    ...
    void Draw(ImVec2 size, float minWidth, float maxWidth)
    {
        ...
        auto hoverCol =
            ImLerp(ImGui::GetStyle().Colors[ImGuiCol_ChildBg], ImColor(28, 35, 43).Value, opacity);
  ...
  public:
    ...
    void Draw()
    {
                         ...
                         ImGuiWindowFlags_NoCollapse);
        {
                        ...
                        for (const auto sub : subs)
                        {
                                                    ...
                                                    ImGuiTabItemFlags_Trailing))
                            {
                                ...
                                ImGui::BeginChild("__SubScrollable##loooooooooool", {}, false);
                                {
                                    ...
                                    auto s =
                                        ImVec2(ImGui::GetContentRegionAvail().x * 0.5f - 7.f, 0);
                                    s.x -= 11;
            ...
            ImGui::BeginGroup();
            {
                                  ...
                                  ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
                {
                    ...
                    // g->AddShadowCircle(bMin + (bSize / 2.f), 7,
                    // colors::accent1, 100.f, {}); g->AddShadowCircle(bMin +
                    // (bSize / 2.f), 7, colors::accent1, 100.f, {});
                                   ...
                                   ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar))
        {
            ...
            const auto txtWidth            = ImGui::CalcTextSize(_("Keybind Setup")).x;
            const auto textCenterPosOffset = (w / 2) - (txtWidth / 2) + 7;

            ImGui::SetCursorPosX(textCenterPosOffset);
            ImGui::TextUnformatted(_("Keybind Setup"));
            ImGui::PopFont();
            ImGui::Dummy({w, 2});
            ...
            {
                if (GetKeyDownHelper((KeyCode)i))
                {
                    // we proceed only if supported key is catched
                    auto str = GetKeyString((KeyCode)i);

                    if (str.empty())
                        continue;

                    (*currentKeyBind).buttonData.key = (KeyCode)i;
