...
static ImVec2 InputTextCalcTextSizeW(
    ImGuiContext* ctx, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL,
...
bool ImGui::ImageButtonEx(
    ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col,
...
bool ImGui::ImageButton(
    const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1,
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    if (label_size.x > 0.0f)
    {
        ...
        RenderTextEllipsis(
            window->DrawList, label_pos, ImVec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y), bb.Max.x, bb.Max.x, label,
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImGuiID popup_id   = ImHashStr(_("##ComboPopup"), 0, id);
        ...
        case ImGuiDataType_S8:
            {
                ...
                bool  r   = SliderBehaviorT<ImS32, ImS32, float>(
                    bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min, *(const ImS8*)p_max, format, flags,
                    out_grab_bb);
                if (r)
                    *(ImS8*)p_v = (ImS8)v32;
                return r;
        ...
        case ImGuiDataType_U8:
            {
                ...
                bool  r   = SliderBehaviorT<ImU32, ImS32, float>(
                    bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min, *(const ImU8*)p_max, format, flags,
                    out_grab_bb);
                if (r)
                    *(ImU8*)p_v = (ImU8)v32;
                return r;
        ...
        case ImGuiDataType_S16:
            {
                ImS32 v32 = (ImS32) * (ImS16*)p_v;
                bool  r   = SliderBehaviorT<ImS32, ImS32, float>(
                    bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)p_min, *(const ImS16*)p_max, format, flags,
                    out_grab_bb);
                if (r)
                    *(ImS16*)p_v = (ImS16)v32;
                return r;
        ...
        case ImGuiDataType_U16:
            {
                ImU32 v32 = (ImU32) * (ImU16*)p_v;
                bool  r   = SliderBehaviorT<ImU32, ImS32, float>(
                    bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags,
                    out_grab_bb);
                if (r)
                    *(ImU16*)p_v = (ImU16)v32;
                return r;
...
static ImVec2 InputTextCalcTextSizeW(
    ImGuiContext* ctx, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining,
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    Text(
        "CurLenW: %d, CurLenA: %d, Cursor: %d, Selection: %d..%d", state->CurLenW, state->CurLenA, stb_state->cursor,
        stb_state->select_start, stb_state->select_end);
    Text("has_preferred_x: %d (%.2f)", stb_state->has_preferred_x, stb_state->preferred_x);
    Text(
        "undo_point: %d, redo_point: %d, undo_char_point: %d, redo_char_point: %d", undo_state->undo_point,
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
        ...
        for (int n = 0; n < STB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            Text(
                "%c [%02d] where %03d, insert %03d, delete %03d, char_storage %03d \"%s\"", undo_rec_type, n,
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const bool hide_prefix =
            (w_item_one <= CalcTextSize((flags & ImGuiColorEditFlags_Float) ? _("M:0.000") : _("M:000")).x);

        //static const char* ids[4]              = {"##X", "##Y", "##Z", "##W"};
        static std::string ids[4]              = {_("##X"), _("##Y"), _("##Z"), _("##W")};
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            {
                value_changed |=
                    DragFloat(ids[n].c_str(), &f[n], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][n].c_str());
            ...
            if (!(flags & ImGuiColorEditFlags_NoOptions))
                OpenPopupOnItemClick(_("context"), ImGuiPopupFlags_MouseButtonRight);
        }
    }
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (alpha)
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), _("#%02X%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
                ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
        else
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), _("#%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
                ImClamp(i[2], 0, 255));
        SetNextItemWidth(w_inputs);
        if (InputText(
                _("##Text"), buf, IM_ARRAYSIZE(buf),
                ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            ...
            if (alpha)
                r = sscanf(
                    p, _("%02X%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2],
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        if (ref_col != NULL)
        {
            ...
            if (ColorButton(
                    _("##original"), ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_inputs)
    {
        if (RadioButton(_("RGB"), (opts & ImGuiColorEditFlags_DisplayRGB) != 0))
            opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayRGB;
        if (RadioButton(_("HSV"), (opts & ImGuiColorEditFlags_DisplayHSV) != 0))
            opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHSV;
        if (RadioButton(_("Hex"), (opts & ImGuiColorEditFlags_DisplayHex) != 0))
    ...
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs)
            Separator();
        if (RadioButton(_("0..255"), (opts & ImGuiColorEditFlags_Uint8) != 0))
            opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Uint8;
        if (RadioButton(_("0.00..1.00"), (opts & ImGuiColorEditFlags_Float) != 0))
    ...
    if (BeginPopup(_("Copy")))
    {
        ...
        char buf[64];
        ImFormatString(
            buf, IM_ARRAYSIZE(buf), _("(%.3ff, %.3ff, %.3ff, %.3ff)"), col[0], col[1], col[2],
        ...
        if (Selectable(buf))
            SetClipboardText(buf);
        ImFormatString(buf, IM_ARRAYSIZE(buf), _("#%02X%02X%02X"), cr, cg, cb);
        if (Selectable(buf))
            SetClipboardText(buf);
        if (!(flags & ImGuiColorEditFlags_NoAlpha))
        {
            ImFormatString(buf, IM_ARRAYSIZE(buf), _("#%02X%02X%02X%02X"), cr, cg, cb, ca);
            if (Selectable(buf))
                SetClipboardText(buf);
        }
        EndPopup();
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            ColorPicker4(_("##previewing_picker"), &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
    if (allow_opt_alpha_bar)
    {
        if (allow_opt_picker)
            Separator();
        CheckboxFlags(_("Alpha Bar"), &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    EndPopup();
}
