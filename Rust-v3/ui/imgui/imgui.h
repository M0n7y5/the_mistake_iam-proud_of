...
namespace ImGui
{
    ...
    IMGUI_API bool ImageButton(
        const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0),
...
struct ImGuiIO
{
    ...
    // - presently Begn()/End() and BeginChild()EndChild() needs to ALWAYS be called in tandem, regardless of return
    // value of BeginXXX()
    //   this is inconsistent with other BeginXXX functions and create confusion for many users.
    // - we expect to update the API eventually. In the meanwhile we provided tools to facilitate checking user-code
    // behavior.
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_ShadowCutOutShapeBackground = 1 << 9 // Do not render the shadow shape under the objects to be shadowed
                                                     // to save on fill-rate or facilitate blending. Slower on CPU.
};
...
struct ImDrawList
{
    ...
    // Shadows primitives
    // [BETA] API
    // - Add shadow for a object, with min/max or center/radius describing the object extents, and offset shifting the
    // shadow.
    ...
    //   Using the ImDrawFlags_ShadowCutOutShapeBackground flag makes the function not render this area and leave a hole
    //   under the object.
    //    - Shadows w/ fill under the object: a bit faster for CPU, more pixels rendered, visible/darkening if used
    //    behind a transparent shape.
    //      Typically used by: small, frequent objects, opaque objects, transparent objects if shadow darkening isn't an
    //      issue.
    //    - Shadows w/ hole under the object: a bit slower for CPU, less pixels rendered, no difference if used behind a
    //    transparent shape.
    //      Typically used by: large, infrequent objects, transparent objects if exact blending/color matter.
    // - FIXME-SHADOWS: 'offset' + ImDrawFlags_ShadowCutOutShapeBackground are not currently supported together with
    // AddShadowCircle(), AddShadowConvexPoly(), AddShadowNGon().
    #define IMGUI_HAS_SHADOWS 1
    IMGUI_API void AddShadowRect(
        const ImVec2& obj_min, const ImVec2& obj_max, ImU32 shadow_col, float shadow_thickness,
        const ImVec2& shadow_offset, ImDrawFlags flags = 0, float obj_rounding = 0.0f);
    IMGUI_API void AddShadowCircle(
        const ImVec2& obj_center, float obj_radius, ImU32 shadow_col, float shadow_thickness,
        const ImVec2& shadow_offset, ImDrawFlags flags = 0, int obj_num_segments = 12);
    IMGUI_API void AddShadowConvexPoly(
        const ImVec2* points, int points_count, ImU32 shadow_col, float shadow_thickness, const ImVec2& shadow_offset,
        ImDrawFlags flags = 0);
    IMGUI_API void AddShadowNGon(
        const ImVec2& obj_center, float obj_radius, ImU32 shadow_col, float shadow_thickness,
        const ImVec2& shadow_offset, ImDrawFlags flags, int obj_num_segments);
...
struct ImFontAtlasShadowTexConfig
{
    ...
    float TexDistanceFieldOffset; // How much to offset the distance field by (allows over/under-shadowing, potentially
                                  // useful for accommodating rounded corners on the "casting" shape).
    ...
    } // The size of the texture area required for the actual 2x2 rectangle shadow texture (after the redundant corners
      // have been removed). Padding is required here to avoid sampling artifacts at the edge adjoining the removed
      // corners.    int     CalcConvexTexWidth() const; // The width of the texture area required for the convex shape
      // shadow texture.
    int GetConvexTexPadding() const
    {
        return 8;
    } // Number of pixels of padding to add to the convex shape texture to avoid sampling artifacts at the edges. This
      // also acts as padding for the expanded corner triangles.
...
struct ImFontAtlas
{
    ...
    ImVec4 ShadowRectUvs[10]; // UV coordinates for shadow textures, 9 for the rectangle shadows and the final entry for
                              // the convex shape shadows
...
struct ImFont
{
    ...
    IMGUI_API void RenderText(
        ImDrawList* draw_list, float size, const ImVec2& pos, ImU32 col, const ImVec4& clip_rect,
