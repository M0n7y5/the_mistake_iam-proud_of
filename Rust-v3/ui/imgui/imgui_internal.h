...
namespace ImStb
{
    ...
    // "Paranoid" Debug Asserts are meant to only be enabled during specific debugging/work, otherwise would slow down
    // the code too much. We currently don't have many of those so the effect is currently negligible, but onward intent
    // to add more aggressive ones in the code.
    ...
    // This will call IM_DEBUG_BREAK() which you may redefine yourself. See https://github.com/scottt/debugbreak for
    // more reference.
...
// Helpers: Maths
IM_MSVC_RUNTIME_CHECKS_OFF
    // - Wrapper for standard libs functions. (Note that imgui_demo.cpp does _not_ use them to keep the code easy to
    // copy)
...
namespace ImGui
{
    ...
    IMGUI_API bool IsKeyPressed(
        ...
        ImGuiInputFlags flags =
            0); // Important: when transitioning from old to new IsKeyPressed(): old API has "bool repeat = true", so
